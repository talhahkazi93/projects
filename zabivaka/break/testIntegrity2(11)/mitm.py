...
def changeMsg(msg):
  ...
  amount = alteredMsg['deposit']
  amount = amount + 100
  alteredMsg['deposit'] = amount
  #print alteredMsg
  encryptedMsg = encryptMsg(alteredMsg)
  return encryptedMsg

def encryptMsg(msg):
  initVector = "1111111111111111"
  key = "test*12345678912";
  paddedMsg = padNew(json.dumps(msg))
  iv_bytes = initVector.encode()
  aes = AES.new(key, AES.MODE_CBC, iv_bytes)
  encryptedMsg = base64.b64encode(aes.encrypt(paddedMsg))
  return encryptedMsg
